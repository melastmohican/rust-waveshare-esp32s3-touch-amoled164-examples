//! # CO5300 QSPI Display Demo Example
//!
//! Drives a CO5300 AMOLED display over hardware QSPI in 1-1-4 mode
//! (instruction/address on 1 line, pixel data on 4 lines) with `embedded-graphics` primitives.
//!
//! ## Hardware
//!
...
//!
//! 1. **Native Resolution (280×456)**: The CO5300 display hardware requires a 280×456 native
//!    framebuffer size to prevent memory wrap-around distortion across QSPI DMA transfers.
//! 2. **2-Line Alignment Constraint**: The CO5300 controller requires `y_alignment = 2`.
...
//! 3. **DMA Payload Limits**: Each 38-line chunk payload is 21,280 bytes (`38 × 280 × 2`), fitting
...
async fn main(_spawner: Spawner) {
    info!("=== CO5300 QSPI Display Example (Waveshare ESP32-S3-Touch-AMOLED-1.64) ===");
    ...
    //   D3:   GPIO14
    //   RST:  GPIO21
    //   CS:   GPIO9 (Rev V1 PCB) or GPIO46 (Rev V2 PCB)
    ...
    // Allocate TX descriptors manually (8 descriptors handle 8 * 4092 = 32736 bytes).
    // Each 57-line chunk is 57 * 280 * 2 = 31,920 bytes.
    ...
    // ── Framebuffer (static, 280×456 RGB565 = 255,360 bytes) ───────────────
    ...
    Rectangle::new(Point::new(20, 70), Size::new(120, 120))
        .into_styled(PrimitiveStyle::with_fill(Rgb565::RED))
        .draw(&mut fb_disp)
        .unwrap();

    Rectangle::new(Point::new(140, 150), Size::new(120, 120))
        .into_styled(PrimitiveStyle::with_fill(Rgb565::GREEN))
        .draw(&mut fb_disp)
        .unwrap();

    Circle::new(Point::new(60, 230), 160)
    ...
    // CO5300 requires 2-line alignment for y_start and (y_end + 1)
    info!("Flushing framebuffer to display (in 12 38-line chunks)...");
